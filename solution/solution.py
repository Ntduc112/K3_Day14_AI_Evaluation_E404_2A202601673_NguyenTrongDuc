...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            ...
            if score_values:
                entry_averages.append(sum(score_values) / len(score_values))
                all_scores.extend(score_values)

        overall_average = sum(all_scores) / len(all_scores) if all_scores else 0.5
        positional_bias = (
            len(entry_averages) > 1
            and all(entry_averages[0] > score for score in entry_averages[1:])
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        fallback = "Review the failure trace and define a corrective action"
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            "hallucination": (
                "Add claim-level grounding checks to reject statements unsupported "
                "by retrieved context"
            ),
            "irrelevant": (
                "Add intent-focused prompt examples so answers address the user's "
                "question directly"
            ),
            "incomplete": (
                "Increase evidence coverage and add answer checklists for required "
                "conditions and exceptions"
            ),
            "off_topic": (
                "Add intent detection and query rewriting before retrieval and generation"
            ),
            "refusal": (
                "Calibrate guardrails with allowed-domain examples to reduce false refusals"
            ),
        }
        categories = self.categorize_failures(failures)
        ranked_types = sorted(
            categories,
            key=lambda failure_type: categories[failure_type],
            reverse=True,
        )
        suggestions: list[str] = []
        for failure_type in ranked_types:
            normalized_type = failure_type.lower()
            suggestion = suggestion_by_type.get(normalized_type)
            if suggestion and suggestion not in suggestions:
                suggestions.append(suggestion)

        fallback_suggestions = [
            "Tune retrieval chunking, top-k, and reranking using failed evidence traces",
            "Add representative failed cases to the golden dataset as regression tests",
            "Review low-scoring answers with human labels and recalibrate thresholds",
        ]
